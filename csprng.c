...
#define VERSION 104 //id for the validation
#define FILTER_TYPE 2// 0=> RSG overworld (Practice Seeds); 1=> village only; 2=> shipwreck only; 3 => jungle only; 4 => coinflip (classic); 5=> whatever I want; 6 => loot testing
#define DEBUG 0
#include <string.h>
#include <time.h>
...
int portalLocation(int64_t seed, int villageMode, int* px, int* pz){
  ...
  if (villageMode == 1){
    if (!valid2 || p2.x >= 80 || p2.z >= 64){// || p2.x >= 144 || p2.z >= 144){
      return 0;
    }
    *px = p2.x;
    *pz = p2.z;
...
int portalLoot(int64_t lower48, int px, int pz){
  ...
  if (fitness >= 400){ // around 5 is 3 iron and flint and steel, around 10 is that plus lots of OBI
...
int spawn_close(int64_t seed, LayerStack* gp){ //costs about 1/6 biomes...
  ...
  if (spawn.x >= 0 && spawn.x <= 80 && spawn.z >= 0 && spawn.z <= 80){
    return 1;
  }
  return 0;
}
